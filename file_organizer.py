...
def music(source_file_path: str) -> bool:
    try:
        ...
        if audio_file.tag == None:
            ...
            artist_filepath = '%s/Music/%s/%s' % (
            get_my_user_folder_path(), audio_file.tag.album_artist, audio_file.tag.album)
...
def read_msword(source_file_path: str) -> str:
    ...
    while i < len(document.paragraphs):
        document_content = document_content + document.paragraphs[i].response
        i = i + 1
...
def organize(directory):
    ...
    try:
        ...
        all_file_path = get_all_file_path(f'%s/{directory}' % (get_my_user_folder_path()))
        i = 0
